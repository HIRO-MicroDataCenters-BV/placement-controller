...
class ResourceSnapshot:
    ...
    def find_resources_by_kind_and_identity(self, kind: str, identity: str) -> List[Dict[str, Any]]:
        resources = self.get_resources_by_kind(kind)
        if resources:
            return [resource for resource in resources if self.get_resource_name(resource) == identity]
        else:
            return []
...
class K8SClient:
    async def fetch_snapshot(self) -> ResourceSnapshot:
        ...
        general_info = await asyncio.gather(self.get_cluster_info(), self.get_api_versions())
