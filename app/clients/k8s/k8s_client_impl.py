...
class K8SClientImpl(K8SClient):
    ...
    async def get_cluster_info(self) -> Dict[str, Any]:
        async def get_cluster_info_internal(
            ...
            configmap_api = await dyn_client.resources.get(api_version="v1", kind="ConfigMap")
            results = await configmap_api.get(namespace="kube-system", name="kubeadm-config")
            if results:
    ...
    async def watch_pods(self, queue: AsyncQueue[K8SEvent]) -> None:
        async def watch_internal(dyn_client: DynamicClient) -> None:
            ...
            async for watch_event in v1_pods.watch(watcher=watcher, timeout=7776000):
                try:
    ...
    async def watch_nodes(self, queue: AsyncQueue[K8SEvent]) -> None:
        async def watch_internal(dyn_client: DynamicClient) -> None:
            ...
            async for watch_event in v1_nodes.watch(watcher=watcher, timeout=7776000):
                try:
                    type = watch_event["type"]
                    object = watch_event["object"]
                    kind = object.kind
